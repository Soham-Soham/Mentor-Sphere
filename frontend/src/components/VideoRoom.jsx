...
const VideoRoom = () => {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        socket.on("user-joined", ({ socketId, name, avatar }) => {
          ...
          // onnegotiationneeded will fire and we will create+send offer
        });
  ...
  const createPeerConnection = (remoteSocketId) => {
    ...
    // important: createOffer in negotiationneeded (ensures tracks already added)
    pc.onnegotiationneeded = async () => {
      console.log("onnegotiationneeded -> creating offer for", remoteSocketId);
      try {
        const offer = await pc.createOffer();
        await pc.setLocalDescription(offer);
        socket.emit("offer", { roomId, offer: pc.localDescription, from: socket.id, to: remoteSocketId });
      } catch (err) {
